...
from .models import( weekend_calendar,assign_weekend,holiday,holiday_calendar,assign_holiday,WeekendDetail,leave_type,leave_entitlement,applicablity_critirea,emp_leave_balance,leave_accrual_transaction,leave_reset_transaction,employee_leave_request,Attendance,Shift,
                     ...
                     AttendanceValidationPolicy,LateComingPolicy,EarlyExitPolicy,EmpAttendancePolicy,AttendancePolicyAssignment
                     )
    ...
    def match_filter_criteria(self, row_data, filter_criteria):
        ...
        for field, values in filter_criteria.items():
            ...
            if row_value is None or str(row_value).strip() not in values:
                return False
        return True
class CompensatoryLeaveAllocationviewset(viewsets.ModelViewSet):
    queryset = CompensatoryLeaveAllocation.objects.all()
    serializer_class = CompensatoryLeaveAllocationSerializer
    # permission_classes = [CompensatoryLeaveAllocationPermission]
    @action(detail=True, methods=['post'])
    def allocate(self, request, pk=None):
        ...
        if not comp_leave_type:
            return Response(
                {"error": "Compensatory leave type not found"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response({
            "message": "Compensatory leave allocated successfully"
        })
...
class LateinEarlyoutApprovalViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
         ...
         if not user.is_authenticated:
             return LateinEarlyoutApproval.objects.none()

         if user.is_superuser:
             return LateinEarlyoutApproval.objects.all()
         
         return LateinEarlyoutApproval.objects.filter(
        Q(approver=user) |
        Q(deligate_to=user, is_deligate=True)
     ).distinct()
    ...
    def delegate(self, request, pk=None):
        ...
        if delegate_user.email:
            ...
            message = f"""
                ...
                Employee        : {approval.lateinearlyout_request.employee}
                Request Type    : {approval.lateinearlyout_request.request_type}
                Status          : {approval.lateinearlyout_request.status}
            ...
            created_notification = send_notification_email(
                ...
                branch=None,
                title="Delegation Assigned",
                message=f"{approval.approver.username} has delegated request {approval.lateinearlyout_request.request_type} to you.",
                delegate_user=approval.approver,
                template_type="request_created",
                context={
                    **get_employee_context(approval.lateinearlyout_request.employee),
                        "request_type": approval.lateinearlyout_request.request_type,
                    },
...
class EmpAttendancePolicyViewset(viewsets.ModelViewSet):
    queryset = EmpAttendancePolicy.objects.all()
    serializer_class = EmpAttendancePolicySerializer
